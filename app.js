...
function Docly() {
  ...
  function buildExportText() {
    const title = gist ? gist.title : file ? file.name : "Docly Summary";
  ...
  function exportAsPdf() {
    ...
    doc.setFontSize(16);
    doc.setTextColor(20);
    doc.text(gist ? gist.title : "Docly Summary", marginLeft, y);
    y += 8;
    doc.setFontSize(10);
    ...
    y += 10;
    if (gist) {
      if (gist.summary) writeSection("TL;DR", gist.summary);
      if (gist.points.length) writeSection("Key Points", gist.points.map((p, i) => `${i + 1}. ${p}`).join("\n"));
      if (gist.important.length) writeSection("Important to Know", gist.important.map(p => `• ${p}`).join("\n"));
  ...
  function exportAsWord() {
    let bodyHtml = "";
    if (gist) {
      bodyHtml += `<h3>TL;DR</h3><p>${gist.summary}</p>`;
      if (gist.points.length) {
        bodyHtml += `<h3>Key Points</h3><ol>${gist.points.map(p => `<li>${p}</li>`).join("")}</ol>`;
      }
      if (gist.important.length) {
        bodyHtml += `<h3>Important to Know</h3><ul>${gist.important.map(p => `<li>${p}</li>`).join("")}</ul>`;
      }
      if (gist.tags.length) {
    ...
    if (messages.length) {
      bodyHtml += "<h3>Q&A</h3>";
      bodyHtml += messages.map(m => `<p><strong>${m.role === "user" ? "You" : "Docly"}:</strong> ${m.display.replace(/\n/g, "<br/>")}</p>`).join("");
    }
    const html = `<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'>
<head><meta charset='utf-8'><title>Docly Summary</title></head>
<body>
<h2>Docly — ${gist ? gist.title : file ? file.name : "Summary"}</h2>
...
${bodyHtml}
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, gist.summary)), gist.points.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "bg-[#16213A] border border-[#25355A] rounded-lg p-4"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, p))))), gist.important.length > 0 && /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-[14px] text-[#E6EDF3] leading-snug"
  }, p))))), gist.tags.length > 0 && /*#__PURE__*/React.createElement("div", {
